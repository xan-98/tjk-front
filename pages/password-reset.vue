            ...
            <div class="login-wrap p-4 p-md-7">
              <div class="d-flex">
                <div class="w-100">
                  <h3 class="mb-4">
                    {{
                      $tr.t("Açar sözüni täzelemek", $store.state.currentLang)
                    }}
                  </h3>
                </div>
              </div>
      ...
      this.$axios
        ...
        .then((res) => {
          if (res.data.status == "success") {
            ...
            this.toast(
              this.$tr.t("Emailiňizi barlaň"),
              this.$tr.t("Üstünlikli iberildi"),
              "success"
            );
          }
        })
        .catch((err) => {
          this.toast(
            this.$tr.t("Email tapylmady"),
            this.$tr.t("Ýalňyşlyk bar")
          );
          console.log(err);
        });
    },
