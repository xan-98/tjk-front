<template>
  <div>
    ...
    <div v-if="$store.state.currentLang != '' && isLoad == true">
      <Header v-show="!$store.getters.getFirst()" />
      <div :class="!$store.getters.getFirst() ? 'pt-55 lgx:pt-30' : ''">
        <Nuxt />
      </div>
      <Footer v-show="!$store.getters.getFirst()" />
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      isFirst: this.$store.getters.getFirst(),
      isLoad: false,
    };
  },
  async fetch() {
